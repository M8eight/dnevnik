...
class AcademicPeriodServiceTest {
    ...
    void getAll_ShouldReturnMappedList() {
        ...
        when(academicPeriodRepository.findAllOrderAsc()).thenReturn(List.of(entity));
        ...
        verify(academicPeriodMapper).toResponse(entity);
    }
    ...
    void findById_ShouldReturnResponse_WhenFound() {
        ...
        assertThat(result.academicYear().id()).isEqualTo(ACADEMIC_YEAR_ID);
    }
    ...
    void create_ShouldReturnCreatedPeriod() {
        ...
        when(academicYearRepository.findById(ACADEMIC_YEAR_ID)).thenReturn(Optional.of(buildAcademicYear()));
    ...
    @Test
    @DisplayName("create — бросает NotFoundException если AcademicYear не найден")
    void create_ShouldThrowNotFoundException_WhenAcademicYearNotFound() {
        AcademicPeriodRequest request = buildRequest();
    ...
    @Test
    @DisplayName("delete — успешно удаляет период")
    void delete_ShouldDeletePeriod() {
        AcademicPeriod entity = buildEntity(false);
        when(academicPeriodRepository.findWithAcademicYearById(PERIOD_ID)).thenReturn(Optional.of(entity));
